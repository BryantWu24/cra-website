...
class BakerySheet extends Component {
  ...
  doAddMaterial = async () => {
    ...
    if (addMaterialInfo.name.trim().length > 0) {
      await axios
        ...
        .catch((e) => {
          this.showSnackbar("error", "新增成分發生異常，請稍後再嘗試。");
        });
  ...
  doSave = async () => {
    ...
    if (Object.values(errorData).some((item) => item.length !== 0))
      this.showSnackbar("error", "請確認資料是否填寫正確");
    else {
          ...
          await axios
            ...
            .then((res) => {
              if (!!res.data) {
                ...
                this.showSnackbar("error", "更新產品發生異常，請稍後再嘗試。");
            })
            .catch((e) => {
              this.showSnackbar("error", "更新產品發生異常，請稍後再嘗試。");
            });
          ...
          await axios
            ...
            .then((res) => {
              if (!!res.data) {
                ...
                this.showSnackbar("error", "新增產品發生異常，請稍後再嘗試。");
            })
            .catch((e) => {
              this.showSnackbar("error", "新增產品發生異常，請稍後再嘗試。");
            });
  ...
  handleAddMaterialChange = ($event) => {
    const addMaterialInfo = this.state.addMaterialInfo;
    if (
      !!this.state.isAddMaterialClicked &&
      addMaterialInfo.name.trim().length === 0
    )
      addMaterialInfo.err_name = "此欄位為必填欄位";
  ...
  handleUpload = (e) => {
    ...
    reader.onloadend = function (e) {
      const data = this.state.data;
      data.FBakeryItemUrl = [reader.result];
      this.setState(data);
    }.bind(this);
    console.log(url);
  };
  ...
  upload = async (e) => {
    if (this.state?.uploadFile !== null) {
      ...
      return await axios
        .post(Config.apiUrl + "/api/upload", data)
        .then((res) => {
          console.log(res);
          if (!!res.data) {
            switch (res.data.code.toString()) {
    ...
    return (
      <Grid container spacing={2}>
        <Snackbar
          ...
          onClose={() => {
            this.handleSnackbarState("close");
          }}
          key={"top center"}
        >
          <Alert
        ...
        <Grid item xs={12} sm={6} md={4} lg={2}>
          <TextField
            ...
            fullWidth
            onChange={($event) => {
              this.handleChange("FBakeryItemName", $event);
            }}
            value={this.state.data.FBakeryItemName}
            error={
              this.state.errorData?.FBakeryItemName?.length > 0 &&
              !!this.state.hasDoSave
            }
          />
          <div style={{ width: "100%", textAlign: "left", color: "red" }}>
            {this.state.errorData.FBakeryItemName}
          </div>
        </Grid>
        ...
        <Grid item xs={12} sm={6} md={4} lg={2}>
          <TextField
            ...
            fullWidth
            onChange={($event) => {
              this.handleChange("FUnitPrice", $event);
            }}
            value={this.state.data.FUnitPrice}
            error={
              this.state.errorData?.FUnitPrice?.length > 0 &&
              !!this.state.hasDoSave
            }
          />
          <div style={{ width: "100%", textAlign: "left", color: "red" }}>
            {this.state.errorData.FUnitPrice}
          </div>
        </Grid>
        <Grid item xs={12} sm={6} md={4} lg={2}>
          <TextField
            ...
            fullWidth
            onChange={($event) => {
              this.handleChange("FStorageCount", $event);
            }}
            value={this.state.data.FStorageCount}
            error={
              this.state.errorData?.FStorageCount?.length > 0 &&
              !!this.state.hasDoSave
            }
          />
          <div style={{ width: "100%", textAlign: "left", color: "red" }}>
            {this.state.errorData.FStorageCount}
          </div>
        </Grid>
        <Grid item xs={12} sm={6} md={4} lg={2}>
          <TextField
            ...
            fullWidth
            onChange={($event) => {
              this.handleChange("FStorageDays", $event);
            }}
            value={this.state.data.FStorageDays}
            error={
              this.state.errorData?.FStorageDays?.length > 0 &&
              !!this.state.hasDoSave
            }
          />
          <div style={{ width: "100%", textAlign: "left", color: "red" }}>
            {this.state.errorData.FStorageDays}
          </div>
        </Grid>
        <Grid item xs={12} sm={6} md={4} lg={2}>
          <TextField
            ...
            fullWidth
            onChange={($event) => {
              this.handleChange("FStorageMethod", $event);
            }}
            value={this.state.data.FStorageMethod}
            error={
              this.state.errorData?.FStorageMethod?.length > 0 &&
              !!this.state.hasDoSave
            }
          />
          <div style={{ width: "100%", textAlign: "left", color: "red" }}>
            {this.state.errorData.FStorageMethod}
          </div>
        </Grid>
        <Grid item xs={12} sm={6} md={4} lg={2}>
          <div
        ...
        <Grid item xs={12} sm={9} md={10} lg={10}>
          <FormControl sx={{ width: "100%" }}>
            ...
            <Select
              ...
              MenuProps={MenuProps}
              error={
                this.state.errorData?.ingredients?.length > 0 &&
                !!this.state.hasDoSave
              }
            >
          ...
          <div style={{ width: "100%", textAlign: "left", color: "red" }}>
            {this.state.errorData.ingredients}
          </div>
        </Grid>
        <Grid item xs={12} sm={3} md={2} lg={2}>
          ...
          >
            <Button
              ...
              startIcon={<AddIcon />}
              onClick={() => {
                this.switchAddMaterialDialog(true);
              }}
              fullWidth
            >
              新增成分
        ...
        <Grid item xs={12} sm={12} md={12} lg={12}>
          <img src={this.state.data.FBakeryItemUrl} style={{ maxHeight: "20rem" }}></img>
        </Grid>
        <Grid item xs={12} sm={12} md={12} lg={12}>
          <div
            style={{
        ...
        {/* 新增成分 Dialog */}
        <Dialog
          open={this.state.isShowAddMaterialDialog}
          disableEscapeKeyDown
          id="add-material-dialog"
        >
          ...
          <DialogContent>
            <TextField
              ...
              error={
                this.state.addMaterialInfo?.err_name?.length > 0 &&
                !!this.state.isAddMaterialClicked
              }
            />
          </DialogContent>
          <DialogActions>
            ...
            >
              ...
              <div>
                <Button color="secondary" onClick={this.doAddMaterial}>
                  建立
                </Button>
              </div>
            </div>
