...
export default class Bakery extends Component {
  ...
  orderListAdd = async (row, index) => {
    ...
    if (orderList[index].FStorageCount > orderList[index].FCount) {
      ...
      orderList[index].FTotalPrice =
        orderList[index].FCount * orderList[index].FUnitPrice;
    ...
    if (this.state.count !== 0) {
      ...
      if (this.state.orderList.length !== 0) {
        ...
        curOrderList = this.state.orderList.map((item) => {
          if (
            item.FBakeryItemId === this.state.currentProductInfo.FBakeryItemId
          ) {
      ...
      if (!isExist) {
        ...
        orderInfo.FBakeryItemName =
          this.state.currentProductInfo.FBakeryItemName;
        ...
        orderInfo.FTotalPrice =
          this.state.currentProductInfo.FUnitPrice * this.state.count;
  ...
  setSelectCountOption = () => {
    ...
    if (ele.length === 0) return <div> 不好意思，商品已全數售出</div>;
    else
      return (
        <FormControl fullWidth>
          ...
          <Select
            ...
            inputProps={{ 'aria-label': 'Without label' }} displayEmpty
  ...
  doCheckOut = async () => {
    ...
    await axios
      .post(Config.apiUrl + "/openapi/bakery/order/create", body)
      .then((res) => {
        console.log(res);
        if (!!res.data) {
  ...
  render() {
    return (
      <div style={{ width: "100%", padding: "0.5rem" }}>
            ...
            this.state.data.map((item, idx) => {
              return (
                ...
                >
                  <BakeryCard
                    data={item}
                    productInfo={this.handleProductInfo}
                  />
                </Grid>
              );
            })
          ...
          <DialogContent>
            {this.state.currentProductInfo.FStorageCount === 0 ? (
              ...
              <div style={{ marginBottom: "1rem" }}>
                單價：$ {this.state.currentProductInfo.FUnitPrice}{" "}
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 總金額：{" "}
                {this.state.currentProductInfo.FUnitPrice * this.state.count} 元
              </div>
            )}
            <div>{this.setSelectCountOption()}</div>
          </DialogContent>
          <DialogActions>
            <Button
              color="secondary"
              onClick={() => {
                this.closeDialog("cart");
              }}
            >
              取消
            </Button>
            {this.state.currentProductInfo.FStorageCount === 0 ? (
              ...
              <Button color="secondary" onClick={this.addCart}>
                加入購物車
              </Button>
            )}
          </DialogActions>
        ...
        {/* 購物清單 Dialog */}
        <Dialog
          open={this.state.isOrderListDialogOpen}
          disableEscapeKeyDown
          id="order-list-dialog"
        >
                        ...
                        >
                          ...
                          <TableCell align="center">
                            ...
                            <IconButton
                              ...
                              component="span"
                              onClick={() => {
                                this.orderListDecrease(row, index);
                              }}
                            >
                              <RemoveIcon />
                            </IconButton>
                            <IconButton
                              ...
                              component="span"
                              onClick={() => {
                                this.orderListDelete(row, index);
                              }}
                            >
                              <DeleteIcon />
                            </IconButton>
            ...
            >
              ...
              <div>
                <Button
                  color="secondary"
                  onClick={() => {
                    this.closeDialog("orderList");
                  }}
                >
                  關閉
                </Button>
                {this.state.orderList.length > 0 ? (
                  <Button color="secondary" onClick={this.doCheckOut}>
                    送出訂單
                  </Button>
                ) : (
                  <div></div>
